...
- Se compara contraseña en texto plano (`usuario.Contrasena == password`).
- `EnsureCreated` + `ALTER TABLE` manual funciona para desarrollo, no sustituye migraciones formales.
- `TrustServerCertificate=True` útil en entornos locales, revisar en producción.

Recomendaciones prioritarias:

1. Implementar hash de contraseñas (por ejemplo, `PasswordHasher<TUser>` o BCrypt).
2. Migrar a EF Core Migrations para control de esquema por ambientes.
3. Mover secretos (`Password`, SMTP) a variables de entorno o Secret Manager.
4. Agregar protección antiforgery explícita en POST si se amplía superficie de formularios.
5. Registrar auditoría de intentos de login y bloqueos.

## 12) Troubleshooting rápido

- **No conecta a SQL Server**
  - Verificar host/puerto/credenciales en `ConnectionStrings:DefaultConnection`.
  - Validar que SQL Server acepte conexiones TCP al puerto configurado.

- **No se envían correos de bloqueo**
  - Revisar `Smtp:Host`, `Smtp:From`, `Smtp:UserName`, `Smtp:Password`.
  - Comprobar firewall/salida al puerto SMTP.

- **Sesión expira antes/después de lo esperado**
  - Confirmar `IdleTimeout` en `Program.cs`.
  - Revisar que llamadas a `KeepAlive` no estén bloqueadas por red/navegador.

- **Cuenta queda bloqueada y tiempo mostrado no coincide**
  - Revisar manejo UTC entre backend (`FormatUnlockAtUtc`) y frontend (`new Date(...)`).
  - Confirmar hora del servidor y del cliente.

## 13) Próximos pasos sugeridos

- Implementar registro de actividad y auditoría de seguridad.
- Añadir pruebas automáticas para login, bloqueo y expiración de sesión.
- Separar capa de servicios (autenticación y notificaciones) para mejorar mantenibilidad.
- Definir estrategia de despliegue (contenedor, variables por ambiente, health checks).

## 14) Notas operativas

- Existe un script SQL de consola en entorno local (`~/.config/JetBrains/.../console.sql`) con `SELECT * FROM Usuarios;` útil para inspección rápida manual.
- El proyecto incluye `RuntimeIdentifier=linux-x64` y `SelfContained=true` en el `.csproj`; ajustar según estrategia de publicación.
